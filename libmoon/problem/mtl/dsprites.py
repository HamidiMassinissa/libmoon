...
from libmoon.problem.mtl.loaders.dsprites_loader import DspritesData
import torch

from libmoon.problem.mtl.objectives import CrossEntropyLoss
from libmoon.problem.mtl.model.dsprites_simple import MultiLeNet
from libmoon.util_global.weight_factor.funs import uniform_pref
from libmoon.util_global.constant import FONT_SIZE

loss_1 = CrossEntropyLoss(label_name='labels_1', logits_name='logits_1')
loss_2 = CrossEntropyLoss(label_name='labels_2', logits_name='logits_2')
...
class DspritesProblem:
    ...
    def __init__(self, args, prefs):
        ...
        self.model_arr = [MultiLeNet([1, 64, 64], 3, 6) for _ in range(self.n_prob)]
        ...
        for pref_idx, pref in enumerate(self.prefs):
            ...
            with torch.no_grad():
                for batch in self.loader_val:
                    ...
                    labels_dict['labels_1'] = data_['labels_1']
                    labels_dict['labels_2'] = data_['labels_2']

                    l1 = loss_1(logits, labels_dict)
                    l2 = loss_2(logits, labels_dict)
                    loss1 += l1
                    loss2 += l2
                    pred1 = logits['logits_1'].max(1).indices
                    pred2 = logits['logits_2'].max(1).indices
                    acc1 += pred1.eq(data_['labels_1']).sum()
                    acc2 += pred2.eq(data_['labels_2']).sum()
                    total += len(data_['labels_1'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--problem', default='dsprites', type=str)  # For attribute in args, we all call problem.
    ...
    problem = DspritesProblem(args, prefs)
