...
class MetaHyperNet(nn.Module):
    ...
    def get_weights_size(self):
        parameters = {
            ...
            "features1.weights": self.ray_hidden_dim * self.ray_hidden_dim,
            "features1.bias": self.ray_hidden_dim,
            "features2.weights": self.ray_hidden_dim * self.ray_hidden_dim,
            "features2.bias": self.ray_hidden_dim,
            "conv0_weights.weights": self.n_kernels * self.kernel_size[0] * self.kernel_size[0] * self.ray_hidden_dim,
            "conv0_weights.bias": self.n_kernels * self.kernel_size[0] * self.kernel_size[0],
            "conv0_bias.weights": self.n_kernels * self.ray_hidden_dim,
            "conv0_bias.bias": self.n_kernels
        }
        ...
        for i in range(1, self.n_conv_layers):
            ...
            parameters[f"conv{i}_weights.weights"] = c * p * self.kernel_size[i] * self.kernel_size[i] * self.ray_hidden_dim
            parameters[f"conv{i}_weights.bias"] = c * p * self.kernel_size[i] * self.kernel_size[i]
            parameters[f"conv{i}_bias.weights"] = c * self.ray_hidden_dim
            parameters[f"conv{i}_bias.bias"] = c

        parameters["hidden0_weights.weights"] = self.target_hidden_dim * 2**i * self.n_kernels * self.latent * self.ray_hidden_dim
        parameters["hidden0_weights.bias"] = self.target_hidden_dim * 2**i * self.n_kernels * self.latent
        parameters["hidden0_bias.weights"] = self.target_hidden_dim * self.ray_hidden_dim
        parameters["hidden0_bias.bias"] = self.target_hidden_dim

        for j in range( self.n_tasks ):
            parameters[f"task{j}_weights.weights"] = self.target_hidden_dim * self.out_dim * self.ray_hidden_dim
            parameters[f"task{j}_weights.bias"] = self.target_hidden_dim * self.out_dim

            parameters[f"task{j}_bias.weights"] = self.out_dim * self.ray_hidden_dim
            parameters[f"task{j}_bias.bias"] = self.out_dim
    ...
    def forward(self, ray, weights):
        features = F.linear(
            ...
            weight=weights["features0.weights"].reshape(
                self.ray_hidden_dim, 2
            ),
            bias=weights["features0.bias"]
        )
        features = F.relu(features)
        features = F.linear(
            features,
            weight=weights["features1.weights"].reshape(
                self.ray_hidden_dim, self.ray_hidden_dim
            ),
            bias=weights["features1.bias"]
        )
        features = F.relu(features)
        features = F.linear(
            features,
            weight=weights["features2.weights"].reshape(
                self.ray_hidden_dim, self.ray_hidden_dim
            ),
            bias=weights["features2.bias"]
        )
        ...
        # conv layers
        out_dict[f"conv0.weights"] = F.linear(
            features,
            weight=weights[f"conv0_weights.weights"].reshape(
                self.n_kernels * self.kernel_size[0] * self.kernel_size[0], self.ray_hidden_dim
            ),
            bias=weights[f"conv0_weights.bias"]
        ) # output_size: 10*9*9
        out_dict[f"conv0.bias"] = F.linear(
            features,
        ...
        for i in range(1, self.n_conv_layers):
            ...
            out_dict[f"conv{i}.bias"] = F.linear(
                ...
                bias=weights[f"conv{i}_bias.bias"]
            )
        
        # hidden layers
        out_dict["hidden0.weights"] = F.linear(
            features,
            weight=weights["hidden0_weights.weights"].reshape(
                self.target_hidden_dim * 2**i * self.n_kernels * self.latent, self.ray_hidden_dim
            ),
            bias=weights["hidden0_weights.bias"]
        )
        out_dict["hidden0.bias"] = F.linear(
            features,
            weight=weights["hidden0_bias.weights"].reshape(
                self.target_hidden_dim, self.ray_hidden_dim
            ),
            bias=weights["hidden0_bias.bias"]
        )

        # task layers
        for j in range( self.n_tasks ):
            out_dict[f"task{j}.weights"] = F.linear(
                features,
                weight=weights[f"task{j}_weights.weights"].reshape(
                    self.target_hidden_dim * self.out_dim, self.ray_hidden_dim
                ),
                bias=weights[f"task{j}_weights.bias"]
            )

            out_dict[f"task{j}.bias"] = F.linear(
                features,
                weight=weights[f"task{j}_bias.weights"].reshape(
                    self.out_dim, self.ray_hidden_dim,
                ),
                bias=weights[f"task{j}_bias.bias"]
            ).flatten()
